console.log('Field.js already loaded')
...
class Field {
    ...
    setImage(field, opacity = 0.5, modify = true, clear = true){
        const { width, height } = defaults.fieldSize
        
        this.segments = field.segments || this.segments
    ...
    drawImage(clear = false, opacity = 0.5){
        ...
        this.ctx.drawImage(img, this.source.offsetX, this.source.offsetY,
            ...
            this.destination.offsetX , this.destination.offsetY,
            fieldSize.width * scale *this.segments, fieldSize.height * scale * this.segments
        )
        // this.drawGrid()
    }
    ...
    import( exporter , opacity = 0.5, modify = true, clear = true){
        const { source } = exporter
        const { width , height } = defaults.fieldSize
        const { cleaner } = defaults
        exporter.source = {
            offsetX : source.x * width,
            offsetY : source.y * height,
        }
        if (exporter.isNotEmpty == false)
            this.setImage( cleaner, false, modify)
        else
            this.setImage( exporter, opacity, modify, clear)
